...
The VQE sweep over $\lambda\in[0,1]$ compares three approaches, in \cref{fig:part-e_vqe_comparison}, where all three curves, exact, VQE, and Qiskit VQE, are indistinguishable. With the precision, the max absolute error for manual VQE and Qiskit VQE are both $\approx 10^{-10}$. 
...
The simplification $W=0$ removes the $H_2$ term, which physically corresponds to a scattering process that conserves the total number of particles in each level individually, in contrast to the $V$ term that scatters pairs across levels. Including $W\neq0$ would add additional $XX+YY$ Pauli strings to the Hamiltonian, enrich the spectral structure, and is expected to further complicate the VQE optimization landscape. It is treated as a natural extension beyond the scope of the present study.
