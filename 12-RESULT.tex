...
The eigenvalues $E_\pm(\lambda)$ are computed both analytically and numerically (using \texttt{numpy.linalg.eigh}) over $101$ values of $\lambda\in[0,1]$, where the maximum discrepancy between the two methods are $\approx10^{-15}$, as shown in \cref{fig:figures/part-b_eigenvalues.pdf}. 
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{Ground state energy as a function of $\lambda$ for three approaches: exact diagonalization, manual VQE, and Qiskit VQE. All three curves are indistinguishable.}
...
\begin{figure}[H]
    ...
    \caption{Absolute error between VQE and exact diagonalization for the two-qubit model, with a maximum of $\approx10^{-10}$ across the full $\lambda$ range.}
...
The Lipkin Hamiltonian is built using the quasispin operators $J_z,\space J_+,\space J_-$ with the pair-scattering interaction $(W=0)$, where the resulting matrices are shown for $J=1$ with $N=2$ particles and $3\times3$ in \cref{h_j=1_matrix} and $J=2$ with $N=4$ particles and $5\times5$ in \cref{h_j=2_matrix}.
...
\begin{equation}
    H_{J=1}=\frac{\epsilon}{2}(ZI+IZ)+\frac{V}{2}(XX-YY)
\end{equation}

For $J=1$, two-qubit mapping. The $4\times4$ Pauli Hamiltonian contains both the $J=1$ tripled $d=3$ and $J=0$ singlet $d=1$. Projecting onto the triplet subspace, spanned by $\ket{11}$ $(m=-1)$, $\frac{\ket{01}+\ket{10}}{\sqrt{2}}$ $(m=0)$ and $\ket{00}$ $(m=+1)$ exactly covers the $3\times3$ quasispin matrix.

For $J=2$, four-qubit mapping. 

$$\boxed{H_{J=2} = \frac{\varepsilon}{2}\sum_{i=1}^{4} Z_i + \frac{V}{2}\sum_{i<j}(X_i X_j - Y_i Y_j)}$$

This gives $4$ single-qubit $Z$ terms and $12$ two-qubit terms ($XX$ and $YY$ for each of the $6$ pairs), totalling $16$ Pauli-strings. The $16\times16$ Hilbert space contains $J=0,1,2$ sectors. The five $J=2$ eigenvalues were extracted from the full Pauli spectrum and verified against the quasispin reference. 

The eigenvalues are computed over $V\in[0,1.5]$ with $\epsilon=1$ and $W=0$.
...
The Hartree-Fock energy of the Lipkin model is \cref{hartree-fock-energy} where $\tilde{V}=\frac{(N-1)V}{\epsilon}$, and the critical interaction strengths are $V_c=1.000$ for $N=2$ ($J=1$) and $V_c=0.333$ for $N=4$ ($J=2$).
...
The ansatz uses alternating entanglement layers, even pairs $(1,0),\space(2,3)$ and odd pairs $(1,2),\space(3,4)$ plus wrap-around to ensure full qubit connectivity after two layers. Multiple random restarts mitigate local minima. 
...
With this we find a maximum error of $\approx10^{-11}$ and $\approx10^{-2}$ for $J=1$ ($N=2$) and $J=2$ ($N=4$) respectively, with mean errors of $\approx10^{-12}$ and $\approx10^{-3}$.
...
For the $J=2$ system, the larger Hilbert space and the more intricate entanglement structure make optimization considerably harder. The residual error of $\approx10^{-2}$ suggests that the hardware-efficient ansatz with depth $4$ and $16$ parameters does not fully capture all correlations in the ground state. Increasing the circuit depth, adding more entanglement layers, or using a symmetry-adapted ansatz that explicitly targets the $J=2$ sector could reduce this error.
